...
const StyledBottomCard = styled(DataCard)<{ dim: any }>`
  ...
  border: 1px solid rgba(29, 3, 82, 0.7);
  box-shadow: 0 0 5px rgba(99, 44, 209, 0.1), 0 0 7px rgba(99, 44, 209, 0.3);
    ...
    <PageWrapper gap="lg" justify="center">
      ...
      {showAddLiquidityButton && (
        <VoteCard>
          ...
          <CardSection>
            <AutoColumn gap="md">
              <RowBetween>
                <TYPE.white fontWeight={600}>Step 1. Get Aetherswap Liquidity tokens</TYPE.white>
              </RowBetween>
              <RowBetween style={{ marginBottom: '1rem' }}>
                <TYPE.white fontSize={14}>
                  {`Aetherswap V1 LP tokens are required. Once you've added liquidity to the ${token0?.symbol}-${token1?.symbol} pool you can stake your liquidity tokens on this page.`}
                </TYPE.white>
              </RowBetween>
              <ButtonPrimary
                padding="8px"
                ...
                width={'fit-content'}
                as={Link}
                to={`/add/v1/${token0 && currencyId(token0)}/${token1 && currencyId(token1)}`}
              >
      ...
      <PositionInfo gap="lg" justify="center" dim={showAddLiquidityButton}>
          ...
          <StyledBottomCard dim={stakingInfo?.stakedAmount?.equalTo(JSBI.BigInt(0))}>
            ...
            <AutoColumn gap="md">
              ...
              <YourDeposits>
                ...
                <TYPE.white>
                  AETHER-V1 {currency0?.symbol}-{currency1?.symbol}
                </TYPE.white>
              </YourDeposits>
          ...
          <DataButtonRow style={{ marginBottom: '1rem' }}>
            {stakingInfo && !isNomad && (
              <ButtonPrimary padding="8px" borderRadius="8px" width="360px" onClick={handleDepositClick}>
                {stakingInfo?.stakedAmount?.greaterThan(JSBI.BigInt(0)) ? 'Deposit' : 'Deposit Aetherswap V1 LP Tokens'}
              </ButtonPrimary>
            )}
        ...
        {!userLiquidityUnstaked || userLiquidityUnstaked.equalTo('0') ? null : (
          <TYPE.main>{userLiquidityUnstaked.toSignificant(6)} Aetherswap V1 LP tokens available</TYPE.main>
        )}
      </PositionInfo>
    </PageWrapper>
  )
