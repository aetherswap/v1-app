...
export default function AddLiquidity({
  ...
  const oneCurrencyIsWETH = Boolean(
    chainId &&
      ((currencyA && currencyEquals(currencyA, WCANTO[chainId])) ||
        (currencyB && currencyEquals(currencyB, WCANTO[chainId])))
  )
  ...
  const handleCurrencyASelect = useCallback(
    (currencyA: Currency) => {
      ...
      if (newCurrencyIdA === currencyIdB) {
        history.push(`/add/v1/${currencyIdB}/${currencyIdA}`)
      } else {
        history.push(`/add/v1/${newCurrencyIdA}/${currencyIdB}`)
      }
    },
  ...
  const handleCurrencyBSelect = useCallback(
    (currencyB: Currency) => {
      ...
      if (currencyIdA === newCurrencyIdB) {
        if (currencyIdB) {
          history.push(`/add/v1/${currencyIdB}/${newCurrencyIdB}`)
        } else {
          history.push(`/add/v1/${newCurrencyIdB}`)
        }
      } else {
        history.push(`/add/v1/${currencyIdA ? currencyIdA : 'ETH'}/${newCurrencyIdB}`)
      }
    },
                  ...
                  <BlueCard>
                    <AutoColumn gap="10px">
                      <TYPE.link fontWeight={400} color={'primaryText1'}>
                        ...
                        The amount of tokens you receive is proportional to your share of the pool, and can be redeemed
                        at any time.
                      </TYPE.link>
                    </AutoColumn>
                  </BlueCard>
