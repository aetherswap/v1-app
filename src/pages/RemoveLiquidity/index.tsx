      ...
      <>
        <RowBetween>
          <Text color={theme.text2} fontWeight={500} fontSize={16}>
            {'AET ' + currencyA?.symbol + '/' + currencyB?.symbol} Burned
          </Text>
          <RowFixed>
  ...
  const handleSelectCurrencyA = useCallback(
    (currency: Currency) => {
      if (currencyIdB && currencyId(currency) === currencyIdB) {
        history.push(`/remove/v1/${currencyId(currency)}/${currencyIdA}`)
      } else {
        history.push(`/remove/v1/${currencyId(currency)}/${currencyIdB}`)
      }
    },
  ...
  const handleSelectCurrencyB = useCallback(
    (currency: Currency) => {
      if (currencyIdA && currencyId(currency) === currencyIdA) {
        history.push(`/remove/v1/${currencyIdB}/${currencyId(currency)}`)
      } else {
        history.push(`/remove/v1/${currencyIdA}/${currencyId(currency)}`)
      }
    },
    [currencyIdA, currencyIdB, history]
  )
                  ...
                  <AutoColumn gap="10px">
                    ...
                    {chainId && (oneCurrencyIsWETH || oneCurrencyIsETH) ? (
                      <RowBetween style={{ justifyContent: 'flex-end' }}>
                        {oneCurrencyIsETH ? (
                          <StyledInternalLink
                            to={`/remove/v1/${currencyA?.isNative ? WCANTO[chainId].address : currencyIdA}/${
                          ...
                          <StyledInternalLink
                            to={`/remove/v1/${
                              currencyA && currencyEquals(currencyA, WCANTO[chainId]) ? 'ETH' : currencyIdA
                            }/${currencyB && currencyEquals(currencyB, WCANTO[chainId]) ? 'ETH' : currencyIdB}`}
                          >
                            Receive CANTO
                          </StyledInternalLink>
                        ) : null}
