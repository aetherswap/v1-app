...
const RowWrapper = styled(Row)<{ bgColor: string; active: boolean }>`
  ...
  padding: 1rem;
  border-radius: 12px;
  border: 1px solid ${({ active, theme }) => (active ? '#400aab' ?? 'transparent' : theme.bg2)};
  color: ${({ active }) => (active ? '#400aab' : 'white')};
`
    ...
    <Wrapper>
      ...
      {tempList && (
        <PaddedColumn style={{ paddingTop: 0 }}>
          <Card backgroundColor={theme.darkTransparent} padding="12px 20px">
            <RowBetween>
              <RowFixed>
